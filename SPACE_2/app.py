...
def calculate_strategy(df):
    ...
    for i in range(1, len(df)):
        ...
        if current_position == 1:
            ...
            # Stop-loss for long: if low <= 0.995 * prev_price, capital * 0.995
            if df['low'].iloc[i] <= 0.995 * prev_price:
                capital = prev_capital * 0.995
            ...
            # Stop-loss for short: if high >= 1.005 * prev_price, capital * 0.995
            if df['high'].iloc[i] >= 1.005 * prev_price:
                capital = prev_capital * 0.995
                df.loc[df.index[i], 'stop_loss_triggered'] = True
        else:
