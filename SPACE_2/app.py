...
def calculate_strategy(df):
    ...
    for i in range(1, len(df)):
        ...
        if current_position == 1:
            ...
            # Stop-loss for long: if low <= entry_price - 0.1 * atr, capital reduced proportionally
            atr_value = df['atr'].iloc[i]
            if df['low'].iloc[i] <= entry_price - 0.1 * atr_value:
                capital = prev_capital * (entry_price - 0.1 * atr_value) / entry_price
            ...
            # Stop-loss for short: if high >= entry_price + 0.1 * atr, capital reduced proportionally
            atr_value = df['atr'].iloc[i]
            if df['high'].iloc[i] >= entry_price + 0.1 * atr_value:
                capital = prev_capital * (2 - (entry_price + 0.1 * atr_value) / entry_price)
                df.loc[df.index[i], 'stop_loss_triggered'] = True
        else:
