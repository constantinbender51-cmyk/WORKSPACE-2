...
def train_model():
    ...
    # Now we have 3 features per time step (close price, SMA_120, and SMA_365)
    ...
    model = Sequential([
        LSTM(16, return_sequences=True, input_shape=(X_train_scaled.shape[1], 3), kernel_regularizer=l1_l2(l1=1e-4, l2=1e-4)),
        Dropout(0.3),
        LSTM(8, return_sequences=False, kernel_regularizer=l1_l2(l1=1e-4, l2=1e-4)),
        Dropout(0.3),
    ...
    # Train the model
    history = model.fit(X_train_scaled, y_train, batch_size=128, epochs=400, validation_data=(X_test_scaled, y_test), verbose=1, callbacks=[ProgressCallback()])
