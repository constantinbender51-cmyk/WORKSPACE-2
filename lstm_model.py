...
def train_model():
    ...
    for i in range(365, len(close_prices)):
        # Features: close prices, SMA_120, and SMA_365 values for past 22 days
        close_features = close_prices[i-22:i]
        sma_120_features = data['sma_120'].values[i-22:i]
        sma_365_features = data['sma_365'].values[i-22:i]
    ...
    model = Sequential([
        LSTM(64, return_sequences=True, input_shape=(X_train_scaled.shape[1], 3)),
        Dropout(0.3),
    ...
    # Train the model
    history = model.fit(X_train_scaled, y_train, batch_size=128, epochs=80, validation_data=(X_test_scaled, y_test), verbose=1, callbacks=[ProgressCallback()])
