...
def train_model():
    ...
    y = []
    skipped_count = 0
    for i in range(366, len(close_prices)):
    ...
    # Train the model
    history = model.fit(X_train_scaled, y_train, batch_size=64, epochs=16, validation_data=(X_test_scaled, y_test), verbose=1, callbacks=[ProgressCallback()])
    ...
    # Align predictions with original datetime indices, starting from row 366
    data_with_predictions = data.copy()
    data_with_predictions['model_output'] = np.nan  # Initialize with NaN
    # Map training predictions back to original indices, offset by 366
    train_indices = range(366, 366 + len(X_train))
    ...
    # Map test predictions back to original indices, offset by 366
    test_indices = range(366 + len(X_train), 366 + len(X_train) + len(X_test))
