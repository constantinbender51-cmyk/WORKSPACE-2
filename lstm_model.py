...
def train_model():
    ...
    # Create sequences of 1 day for features
    X = []
    y = []
    skipped_count = 0
    for i in range(365, len(close_prices)):
        # Features: close prices, SMA_120, and SMA_365 values for the current day
        close_features = close_prices[i-1:i]
        sma_120_features = data['sma_120'].values[i-1:i]
        sma_365_features = data['sma_365'].values[i-1:i]
